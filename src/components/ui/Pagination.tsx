...
export default function Pagination({ totalPages }: { totalPages: number }) {
    ...
    const PaginationNumber = ({
        ...
        const className = clsx('flex h-10 w-10 border-primary items-center justify-center text-sm border', {
            'rounded-l-sm': position === 'first' || position === 'single',
            'rounded-r-sm': position === 'last' || position === 'single',
            'z-10 bg-primary text-secondary': isActive,
            'hover:bg-red-100': !isActive && position !== 'middle',
        ...
        <div className="inline-flex">
            ...
            <div className="flex -space-x-px">
                {allPages.map((page, index) => {
                    ...
                    if (allPages.length - 1 === 1) position = 'single';
                    if (page === '...') position = 'middle';

                    return (
                        <PaginationNumber key={index} page={page} href={createPageURL(page)} position={position} isActive={currentPage === page} />
                    );
                })}
            </div>
